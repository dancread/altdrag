...
int HookSystem() {
  ...
  if (!hinstDLL) {
    ...
    if (hinstDLL == NULL) {
      Error(L"LoadLibrary('hooks_x64.dll')", L"This probably means that the file hooks_x64.dll is missing. You can try reinstalling "APP_NAME".", GetLastError());
      return 1;
    }
  }
  ...
  if (!keyhook) {
    ...
    if (procaddr == NULL) {
      Error(L"GetProcAddress('LowLevelKeyboardProc')", L"This probably means that the file hooks_x64.dll is from an old version or corrupt. You can try reinstalling "APP_NAME".", GetLastError());
      return 1;
    }
  ...
  if (!msghook) {
    ...
    if (procaddr == NULL) {
      Error(L"GetProcAddress('CallWndProc')", L"This probably means that the file hooks_x64.dll is from an old version or corrupt. You can try reinstalling "APP_NAME".", GetLastError());
      return 1;
    }
...
int UnhookSystem() {
  ...
  if (Unload == NULL) {
    #ifdef DEBUG
    Error(L"GetProcAddress('Unload')", L"This probably means that the file hooks_x64.dll is from an old version or corrupt. You can try reinstalling "APP_NAME".", GetLastError());
    #endif
  }
  else {
    Unload();
  ...
  if (hinstDLL) {
    ...
    if (FreeLibrary(hinstDLL) == 0) {
      #ifdef DEBUG
      Error(L"FreeLibrary()", L"Could not free hooks_x64.dll. Try restarting "APP_NAME".", GetLastError());
      #endif
      return 1;
    }
    hinstDLL = NULL;
