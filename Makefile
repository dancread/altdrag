...
LIBS=-mwindows -lshlwapi -lwininet -lcomctl32 -lole32
...
dll:
	...
	$(CC) -g -o dll/hooks.dll dll/hooks.o src/dll/hooks.c src/dll/winenum.c src/core/error.c src/dll/dll_utils.c $(DLL_LIBS)
exec:
	mkdir -p bin
	$(RES_C) resource/altdrag.rc bin/altdrag.o
	#$(CC) -o bin/autostart.o src/config/autostart.c
	#$(CC) -o bin/error.o  src/core/error.c
	$(CC) -g -w -o bin/AltDrag.exe bin/altdrag.o src/core/altdrag.c src/core/tray.c src/core/error.c src/config/config.c src/config/autostart.c src/config/update.c src/lang/languages.c src/lang/localization.c $(LIBS)
tools:
	mkdir -p bin
